  ...
  array.forEach((element) => {
    ...
    if (afterLs) {
      if (element.includes(keys.dir)) {
        ...
        dirTree[filePath[filePath.length - 1]].subDirs.push(dirName[1]);
      } else {
        let splitFileInfo = element.split(" ");
        let currentFiles = [];
        dirTree[filePath[filePath.length - 1]].files.forEach((file) => {
  ...
  allDirKeys.map((eachKey) => {
    ...
    if (dirTree[eachKey].subDirs.length > 0) {
      ...
      while (thisDirSubDir.length > 0) {
        dirSizeTotal =
          dirSizeTotal + calculateFileSizes(dirTree[thisDirSubDir[0]]);
        if (dirTree[thisDirSubDir[0]].subDirs.length > 0) {
          // allSubDirs.push(
          // 	dirTree[thisDirSubDir[0]].subDirs
          // );

          dirTree[thisDirSubDir[0]].subDirs.forEach((subDir) => {
            thisDirSubDir.push(subDir);
          });
        }
        thisDirSubDir.shift();
